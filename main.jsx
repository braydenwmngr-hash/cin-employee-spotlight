...
function Board() {
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => setSlide(s => (s + 1) % 3), content.settings.rotationSeconds * 1000);
...
function Panel({title, items, mode}) { return <article className="card panel"><p className="eyebrow">{title}</p>{items.slice(0,3).map((it,i)=><div className={`panel-item ${mode}`} key={it.id||i}>{it.image ? <img src={it.image}/> : <div className="icon">{mode==='promo'?'🍿':mode==='recognition'?'⭐':'🆕'}</div>}<div><b>{it.label || it.type || it.name || it.from}</b><h3>{it.title || it.name}</h3><p>{it.body}</p></div></div>)}</article>}
...
function Admin() {
  ...
  async function upload(file, cb){ const fd=new FormData(); fd.append('file',file); const r=await fetch('/api/upload',{method:'POST',body:fd}); const j=await r.json(); if(j.file) cb(j.file.url); }
  const tabs = ['Spotlight','Ticker','Promos','What’s New','Recognition','Events','Safety + KPIs','Settings'];
  return <div className="cms"><aside><h2>Cinergy CMS</h2>{tabs.map(t=><button className={tab===t?'active':''} onClick={()=>setTab(t)} key={t}>{t}</button>)}<button onClick={save} className="save">Save & Push Live</button><button onClick={logout}>Log Out</button><a href="/">View Board</a><p>{status}</p></aside><section className="editor"><h1>{tab}</h1>{tab==='Spotlight'&&<SpotlightEditor c={content} set={setContent} upload={upload}/>} {tab==='Ticker'&&<ListEditor c={content} set={setContent} field="ticker" fields={['priority','text']}/>} {tab==='Promos'&&<ListEditor c={content} set={setContent} field="promos" fields={['label','title','body','image']} upload={upload}/>} {tab==='What’s New'&&<ListEditor c={content} set={setContent} field="whatsNew" fields={['type','title','body']}/>} {tab==='Recognition'&&<ListEditor c={content} set={setContent} field="recognition" fields={['from','name','body']}/>} {tab==='Events'&&<ListEditor c={content} set={setContent} field="events" fields={['date','time','title','location']}/>} {tab==='Safety + KPIs'&&<SafetyKpis c={content} set={setContent}/>} {tab==='Settings'&&<Settings c={content} set={setContent} upload={upload}/>}</section><section className="preview"><BoardPreview content={content}/></section></div>
}
...
function ListEditor({c,set,field,fields,upload}){const list=c[field]||[]; return <div>{list.map((item,i)=><div className="edit-card" key={item.id||i}><label className="check"><input type="checkbox" checked={item.active!==false} onChange={e=>update(set,n=>n[field][i].active=e.target.checked)}/> Active</label>{fields.map(f=><Field key={f} label={f} value={item[f]} textarea={f==='body'||f==='text'} onChange={v=>update(set,n=>n[field][i][f]=v)}/>)}{fields.includes('image')&&upload?<input type="file" accept="image/*,video/*" onChange={e=>upload(e.target.files[0],url=>update(set,n=>n[field][i].image=url))}/>:null}<button onClick={()=>update(set,n=>n[field].splice(i,1))}>Remove</button></div>)}<button onClick={()=>update(set,n=>n[field].push({id:uid(),active:true}))}>+ Add Item</button></div>}
